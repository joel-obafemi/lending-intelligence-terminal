...
const ARTICLE_SECTIONS = [
  ...
  {
    header: "SparkLend's stablecoin rates carry a Sky-anchored base",
    paras: [
      "Five of SparkLend's 18 reserves, all stablecoins, price their borrow rate off a utilization curve whose base rate is anchored to a Sky-linked rate source rather than fixed at listing, so the rate responds to both pool utilization and Sky governance. On July 23 at 14:43 UTC, Sky's weekly Atlas Edit executive spell executed on-chain and cut the Sky Base Rate, the wholesale cost of the USDS inventory SparkLend lends against as a Sky Agent, from 3.90% to 3.72%, an 18 bps step. SparkLend's on-chain USDC retail borrow rate stepped from 4.42% to 4.28% in the same transaction cycle, passing through 14 of the 18 bps.",
      "The five Sky-linked reserves are DAI, USDC, USDT, USDS, and PYUSD. On-chain reads show all five point their variable-rate model at a single Sky-linked rate source contract at 0x57027B62, which references sUSDS and supplies the curve's base rate. The step sizes varied by reserve: DAI and USDS moved 18.4 bps with utilization roughly flat, USDC 13.8 bps, PYUSD 8 bps, and USDT 48.6 bps because its utilization also fell from 93.0% to 83.6% that day. SparkLend's other 13 reserves, the collateral book of WETH, wstETH, WBTC and the rest, run the same Aave-shape utilization curves but with base rates fixed at listing, so they did not move on July 23. SparkLend is a hybrid: a Sky-anchored stablecoin book and a listing-fixed collateral book, both clearing on utilization curves.",
      "An earlier step on July 6 cut USDC from 4.61% to 4.42%, but the Base Rate did not move that day and the other four Sky-linked stablecoins held. July 6 was SparkLend adjusting its own markup over the wholesale rate; July 23 was Sky moving the wholesale anchor. SparkLend's markup over the Sky Base Rate widened from 0.52 pp on July 22 to 0.56 pp on July 23, since it passed through 14 of the 18 bps and absorbed the rest. The mechanism matters for any cross-protocol rate read. Comparing Fluid's 4.66% USDC supply APY to SparkLend's 3.47% is not a clean like-for-like: SparkLend's rate carries a Sky-anchored base on top of its utilization curve, so it can shift on days its utilization has not.",
    ],
  },
  {
...
const MINI_PARAS = [
  ...
  "The July State of DeFi Lending on Ethereum report covers four findings from the month: the collateral inflows carrying Aave while its USDC book drains, Sentora overtaking Steakhouse as Morpho's largest curator, SparkLend's stablecoin rates responding to both pool utilization and Sky governance, and the compression of Fluid's June rate premium on USDC.",
  "Full report below.",
]
