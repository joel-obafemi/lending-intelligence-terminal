...
/** Generate the auto-line beneath the verdict strip. Single sentence,
 *  punchy, KOL-quotable. */
export function revenueVerdictSentence(v: RevenueVerdict): string | null {
  if (v.sectorFees30d <= 0) return null
  const parts: string[] = []
  // Sector framing.
  if (v.sectorMom.changePct != null) {
    const dirWord = v.sectorMom.changePct >= 0 ? "+" : ""
    parts.push(
      `Sector earned ${formatCompactUsd(v.sectorFees30d)} over the past 30 days, ${dirWord}${v.sectorMom.changePct.toFixed(0)}% MoM.`,
    )
  } else {
    parts.push(`Sector earned ${formatCompactUsd(v.sectorFees30d)} over the past 30 days.`)
  }
  if (v.sectorCaptureRate > 0) {
    const depositorShare = (1 - v.sectorCaptureRate) * 100
    parts.push(
      `Depositors captured ${depositorShare.toFixed(0)}% of fees; protocol/holders kept ${(v.sectorCaptureRate * 100).toFixed(0)}%.`,
    )
  }
  // Biggest mover with a meaningful prior window.
  const topMover = v.perProtocolMom.find((p) => p.changePct != null)
  if (topMover && topMover.changePct != null && Math.abs(topMover.changePct) >= 5) {
    const verb = topMover.changePct >= 0 ? "led growth at" : "declined"
    const pct = Math.abs(topMover.changePct).toFixed(0)
    parts.push(
      topMover.changePct >= 0
        ? `${topMover.name} ${verb} +${pct}% MoM.`
        : `${topMover.name} ${verb} ${pct}% MoM.`,
    )
  }
  return parts.join(" ")
