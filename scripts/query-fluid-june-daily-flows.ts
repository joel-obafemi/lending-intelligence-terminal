/**
 * One-shot — fluid daily constant-price net supply flow for June 2026,
 * with a WEETH-specific column. For Issue 002 §05's rsETH-unpause /
 * WETH-LTV-restoration concentration thesis.
 *
 *   npm run query:aave-may-daily-flows
 *
 * Output: content/snapshots/2026-06-fluid-daily-flows.csv (CSV, header
 ...
 *   data path:
 *
 *   1. Pull fluid protocol history from DefiLlama (chainTvls.Ethereum
 ...
 *   4. WEETH-specific column applies the same math to the WEETH key only.
...
// ─── Constants ───────────────────────────────────────────────────────────
const PROTOCOL_DEFILLAMA_SLUG = "fluid-lending"
const JUNE_FIRST_UTC = "2026-06-01T00:00:00Z"
const JUNE_LAST_UTC = "2026-06-30T23:59:00Z"
// The thesis split: rsETH unpause was June 14; WETH LTV restoration was
// June 17. Both fall in the second half of the month, so split June 1-13
// from June 14-30 to compare cumulative net flow on either side.
const SPLIT_DAY = "2026-06-14"
const WEETH_KEY = "WEETH"
const OUTPUT_PATH = "content/snapshots/2026-06-fluid-daily-flows.csv"
...
async function main(): Promise<void> {
  ...
  console.log(`fluid June 2026 daily flows (constant-price methodology)`)
  console.log("")

  console.log("[1/3] Fetching DefiLlama fluid protocol history …")
  ...
  // ─── Build June 2026 day-by-day rows ────────────────────────────────
  // We need the previous day (May 31) as the baseline for June 1's delta.
  const days: string[] = []
  for (let d = 0; d < 30; d++) {
    const date = new Date(`${JUNE_FIRST_UTC}`)
    date.setUTCDate(date.getUTCDate() + d)
    days.push(date.toISOString().slice(0, 10))
  }
  const baselineDay = "2026-05-31"

  console.log(`[3/3] Building daily flow rows for June 2026 …`)
  interface DailyRow {
    date_utc: string
  ...
  console.log("── Top 3 outflow days in June ──────────────────────────────")
  ...
  console.log("")
  console.log("── Top 3 inflow days in June (for symmetry) ───────────────")
  ...
  // Cumulative split: June 1-13 vs June 14-30.
  ...
  console.log("── Pre- / post-rsETH-unpause split ─────────────────────────")
  console.log(
    `  Cumulative net flow June 1-13  (pre-unpause):  ${fmtUsd(cumEarly)}` +
      `  (of which WEETH: ${fmtUsd(cumEarlyWeeth)})`,
  )
  console.log(
    `  Cumulative net flow June 14-30 (post-unpause): ${fmtUsd(cumLate)}` +
      `  (of which WEETH: ${fmtUsd(cumLateWeeth)})`,
  )
  const totalMay = cumEarly + cumLate
  if (totalMay !== 0 && cumLate !== 0) {
    const sharePost = (cumLate / totalMay) * 100
    console.log(`  Post-unpause share of net flow: ${sharePost.toFixed(1)}%`)
  }
  console.log("")
