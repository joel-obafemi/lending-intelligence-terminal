 ...
 * May 31 and June 30, 2026. For Issue 002 §06.1's rsETH-unpause-driven
 * LRT outflow attribution.
 *
 *   npm run query:aave-weeth-by-market
 *
 ...
 * coverage shifts.
 *
 * Output: content/snapshots/2026-06-aave-weeth-by-market.json
 */
...
// ─── Targets ─────────────────────────────────────────────────────────────
const MAY31_TARGET_UTC = "2026-05-31T23:59:00Z"
const JUNE30_TARGET_UTC = "2026-06-30T23:59:00Z"
const MAX_DATAPOINT_DELTA_HOURS = 48
const OUTPUT_PATH = "content/snapshots/2026-06-aave-weeth-by-market.json"
...
interface MarketSnapshot {
  ...
  pool_id: string | null
  may31: { timestamp_iso: string; tvl_usd: number; delta_from_target_sec: number } | null
...
async function main(): Promise<void> {
  const mayTarget = Math.floor(new Date(MAY31_TARGET_UTC).getTime() / 1000)
  const juneTarget = Math.floor(new Date(JUNE30_TARGET_UTC).getTime() / 1000)
  const cap = MAX_DATAPOINT_DELTA_HOURS * 3600

  console.log("Aave V3 WEETH by market — May 31 vs June 30, 2026")
  console.log("")
  ...
  for (const { market, pools } of perMarketPools) {
    ...
    const apr = pickClosest(valid, mayTarget)
    const may = pickClosest(valid, juneTarget)
    const mayOk = apr != null && Math.abs(apr.timestamp - mayTarget) <= cap
    const juneOk = may != null && Math.abs(may.timestamp - juneTarget) <= cap
    const maySnap = mayOk
      ? {
          timestamp_iso: new Date(apr!.timestamp * 1000).toISOString(),
          tvl_usd: apr!.tvlUsd,
          delta_from_target_sec: apr!.timestamp - mayTarget,
        }
      : null
    const juneSnap = juneOk
      ? {
          ...
          tvl_usd: may!.tvlUsd,
          delta_from_target_sec: may!.timestamp - juneTarget,
        }
      : null
    const changeUsd =
  ...
  const totalApr =
    okSnaps.reduce((s, m) => s + (m.may31?.tvl_usd ?? 0), 0) || null
  const totalMay =
  ...
  const dominantMay = okSnaps.reduce((max, m) =>
    (m.may31?.tvl_usd ?? 0) > (max?.may31?.tvl_usd ?? 0) ? m : max,
  okSnaps[0])
  ...
  const mayShare =
    totalApr && totalApr > 0
      ? (dominantMay?.may31?.tvl_usd ?? 0) / totalApr
      : null
  const juneShare =
    totalMay && totalMay > 0
      ? (dominantJune?.june30?.tvl_usd ?? 0) / totalMay
      : null
  const concentrationFinding =
    mayShare != null && juneShare != null && mayShare >= 0.95 && juneShare >= 0.95
      ? {
          ...
          dominant_market_share_may31_pct: Number((mayShare * 100).toFixed(2)),
          dominant_market_share_june30_pct: Number((juneShare * 100).toFixed(2)),
  ...
  const output = {
    ...
    totals: {
      may31_tvl_usd: totalApr,
      june30_tvl_usd: totalMay,
  ...
  for (const s of snapshots) {
    ...
    console.log(
      `  ${s.display_name.padEnd(34)} ` +
        `May 31 ${fmtUsd(s.may31?.tvl_usd ?? null).padStart(9)} → ` +
