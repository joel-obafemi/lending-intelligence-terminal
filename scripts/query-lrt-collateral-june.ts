/**
 * LRT collateral across all 6 protocols at May 31 and June 30, 2026 —
 * for Issue 002 §05 cross-protocol context and §08 risk indicators.
 *
 *   npm run query:lrt-collateral
 *
 ...
 * the price effect.
 *
 * Output: content/snapshots/2026-06-lrt-collateral.json
 */
...
interface ProtocolEntry {
  ...
  may_31: ProtocolSnapshot
  june_30: ProtocolSnapshot
}

async function main(): Promise<void> {
  const mayTs = dayKeyToTs(MAY_31_UTC)
  const juneTs = dayKeyToTs(JUNE_30_UTC)
  console.log(`LRT collateral across ${PROTOCOLS.length} protocols at ${MAY_31_UTC} vs ${JUNE_30_UTC}`)
  console.log("")
  ...
  for (const { protocol, history } of histories) {
    ...
    const may31 = snapshotAt(series, mayTs)
  ...
  for (const e of protocols) {
    for (const a of LRT_ASSETS) {
      sectorApr[a] += e.may_31.by_lrt[a]
      sectorMay[a] += e.june_30.by_lrt[a]
    }
  }
  ...
  for (const e of protocols) {
    const has = Object.entries(e.june_30.by_lrt).filter(([, v]) => v > 0)
    if (has.length === 0) {
  ...
  const out = {
    source: {
      ...
      target_dates_utc: { may_31: MAY_31_UTC, june_30: JUNE_30_UTC },
