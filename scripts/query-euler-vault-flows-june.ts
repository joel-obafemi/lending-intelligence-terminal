 ...
 * outflow in May 2026, ranked. For Issue 003 §06.6.
 *
...
// ─── Constants ───────────────────────────────────────────────────────────
const START_TARGET_UTC = "2026-06-01T00:00:00Z"
const END_TARGET_UTC = "2026-06-30T23:59:00Z"
const OUTPUT_PATH = "content/snapshots/2026-06-euler-vault-flows.json"
const TOP_N = 5
...
async function main(): Promise<void> {
  ...
  const flows: VaultFlow[] = await mapConcurrent(
    ...
    async (p) => {
      const base: Omit<
        VaultFlow,
        "may1" | "may31" | "abs_delta_usd" | "pct_delta" | "excluded_reason"
      > = {
        pool_id: p.pool,
      ...
      const may31 = pickClosest(chart, endTarget)
      const may1Delta = may1 ? may1.timestamp - startTarget : null
      const may31Delta = may31 ? may31.timestamp - endTarget : null
      ...
      if (!may1 || Math.abs(may1Delta!) > maxDeltaSec) {
        return {
          ...
          may31: may31
            ? {
                timestamp_iso: new Date(may31.timestamp * 1000).toISOString(),
                tvl_usd: may31.tvlUsd,
                delta_from_target_sec: may31Delta!,
              }
            : null,
          abs_delta_usd: null,
          pct_delta: null,
      ...
      if (!may31 || Math.abs(may31Delta!) > maxDeltaSec) {
        return {
          ...base,
          may1: {
      ...
      const absDelta = may31.tvlUsd - may1.tvlUsd
  ...
  const topOutflows: RankedRow[] = [...ranked]
    ...
    .map((f, i) => ({
      ...
      may31_supply_usd: f.may31!.tvl_usd,
  ...
  const topByCurrentTvl = [...flows]
    ...
    .map((f, i) => ({
      ...
      may31_supply_usd: f.may31?.tvl_usd ?? null,
      change_usd: f.abs_delta_usd,
      change_pct: f.pct_delta,
    }))
  ...
  if (!usingFallback) {
    console.log(`── Top ${TOP_N} OUTFLOW vaults (May 2026) ──────────────────────`)
    for (const r of topOutflows) {
      console.log(
        `  ${String(r.rank).padStart(2)}. ${r.display_name.padEnd(28)}  ` +
          `${fmtUsd(r.may1_supply_usd).padStart(9)} → ${fmtUsd(r.may31_supply_usd).padStart(9)}  ` +
          `Δ ${fmtUsd(r.change_usd).padStart(9)} (${fmtPct(r.change_pct).padStart(7)})`,
      )
    }
    console.log("")
    console.log(`── Top ${TOP_N} INFLOW vaults (May 2026) ───────────────────────`)
