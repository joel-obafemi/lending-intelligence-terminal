 ...
 * roughly Nov 2024 → June 2026. Anything before that is "out of window"
 * and we'll surface that distinction explicitly in the JSON.
 *
 * Output: content/snapshots/2026-06-rys-historical.json
 */
...
const OUTPUT_PATH = "content/snapshots/2026-06-rys-historical.json"
const TARGET_DAY_UTC = "2026-06-30"
...
async function main(): Promise<void> {
  ...
  // Walk backward from the day BEFORE june30 looking for spread >= 0.
  ...
  // ─── Step 3 — 12 monthly readings, June 2025 → June 2026 ──────────────
  ...
  for (let i = 12; i >= 0; i--) {
    // walk back i months from June 2026
  ...
  if (priorPositive) {
    ...
    if (priorYear < "2026") {
      framing = `June 30, 2026's ${fmtPp(junePoint.spreadPct)} print is the first positive Real Yield Spread reading of 2026. The previous positive print was ${priorDay} (${fmtPp(priorPositive.spreadPct)}); the spread was inverted for ${invertedDays} consecutive days between them.`
    } else {
      framing = `June 30, 2026's ${fmtPp(junePoint.spreadPct)} print follows a prior positive print earlier in 2026 (${priorDay}, ${fmtPp(priorPositive.spreadPct)}). The "first positive of 2026" framing does NOT hold — use "first positive in ${invertedDays} days" or "first positive since ${priorDay}" instead.`
    }
  } else if (reachedWindowStart) {
    framing = `June 30, 2026's ${fmtPp(junePoint.spreadPct)} print is the first positive Real Yield Spread reading in the full captured window (${seriesStartDay} → ${seriesEndDay}). The spread has been NEGATIVE for the entire series — "first positive of 2026" is conservative; "first positive since at least ${seriesStartDay}" is the strongest claim defensible from this data.`
  } else {
