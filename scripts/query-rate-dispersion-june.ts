/**
 * One-shot — cross-protocol supply APY dispersion at June 30, 2026 for
 * USDC, USDT, and WETH across the six protocols. For Issue 002 §03.
 *
 *   npm run query:rate-dispersion-may
 *
 ...
 *   - "ratio" = june30_dispersion / twelve_month_average. >1 means June 30
 ...
 *   excluded_protocols.
 *
 * Output: content/snapshots/2026-06-rate-dispersion.json
 */
...
const OUTPUT_PATH = "content/snapshots/2026-06-rate-dispersion.json"
...
interface DispersionRow {
  ...
  ratio_june30_vs_12mo_avg: number | null
}
...
async function main(): Promise<void> {
  ...
  for (const asset of ASSETS) {
    ...
    perAsset.push({
      ...
      ratio_june30_vs_12mo_avg: ratio,
    })
  }
  ...
  for (const a of perAsset) {
    const ratioStr =
      a.ratio_june30_vs_12mo_avg != null
        ? `${a.ratio_june30_vs_12mo_avg.toFixed(2)}×`
        : "—"
    const excl = a.june30.excluded_protocols.length
      ? `  excluded: ${a.june30.excluded_protocols.join(", ")}`
      : ""
    console.log(
      `  ${a.asset.padEnd(6)} ${fmtBps(a.june30.dispersion_bps).padStart(10)}  ` +
        `${fmtBps(a.twelve_month.avg_dispersion_bps).padStart(10)}  ` +
        `${ratioStr.padStart(8)}  ` +
        `${a.june30.per_protocol.length}/6${excl}`,
    )
  }
  console.log("")
  ...
  for (const a of perAsset) {
    console.log(`  ${a.asset}`)
    for (const p of [...a.june30.per_protocol].sort(
    ...
    if (a.june30.excluded_protocols.length > 0) {
      console.log(`    excluded: ${a.june30.excluded_protocols.join(", ")}`)
    }
  }
  console.log("")
