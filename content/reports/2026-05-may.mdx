...
theme: "The Curator Moves"
tagline: "How a single curator's cross-protocol reallocation reshaped two lending protocols in thirty days, while the sector's stablecoin yield closed at parity with T-bills for the first time in eight months."
reading_time_min: 35
...
<DataTable
  ...
  rows={[
    ...
    { indicator: "Liquidation effective penalty (trailing 90d)", reading: "Aave V3 7.0%, SparkLend 4.87%, Morpho 24.10%, Fluid 1.68%; Compound + Euler not yet covered" },
    ...
    { indicator: "Aave V3 USDC market", reading: "97.6% utilization, 9.62% supply APY (borrow squeeze, see §03)" }
  ]}
/>
...
<Lead>Sentora moved $128M of depositor capital between Euler and Morpho. The Morpho curator HHI rose to 3,103. The sector's stablecoin yield closed at parity with T-bills for the first time in eight months.</Lead>
...
What pushed on-chain rates up was the Aave V3 USDC dynamic that Issue 001 first surfaced. The USDC market on Aave V3 ended May at 97.6 percent utilization with 9.62 percent supply APY against a 10.95 percent borrow APY. At that utilization the market sits past its kink, where the interest rate model's slope steepens sharply: each additional unit of borrow demand at the margin pushes supply rate up at multiple times the rate it would in a normally-utilized market. The Aave V3 USDC borrow squeeze is the loudest stablecoin rate signal in DeFi lending at May close, and it is the single largest contribution to the blended-stable APY lift that closed the spread.

Cross-protocol dispersion tells the same story from a different angle. The May 31 dispersion in USDC supply APY across the six protocols was 330.5 basis points, against a 12-month average of 249.3 basis points, a 1.33-multiple. Fluid USDC sat at 5.93 percent at the top of the range; Euler V2 USDC at 2.62 percent at the bottom. The same dollar of USDC earned 331 basis points more on the most generous of the six protocols than on the least. By contrast, USDT and WETH dispersion are both below their 12-month baselines this month, at 161.7 basis points (0.87× baseline) and 35.2 basis points (0.26× baseline). The persistence of USDC dispersion above baseline while the other two assets have converged below it diagnoses an asset-specific dynamic: USDC at Aave V3 is doing something the rest of the stable and ETH markets are not.
...
The cleaner forward question is which mechanism would close the parity reading in either direction. The spread pushing positive on a sustained basis requires either continued borrow-side pressure at the protocol level (the Aave V3 USDC squeeze persisting or extending to other markets) or compression on the unborrowed-supply side (depositors continuing to exit idle capital). The spread pushing negative again requires utilization easing or T-bill yields rising. Neither catalyst is currently visible in the data. The most likely June outcome, absent a new shock, is the spread oscillating around parity until one of those two structural conditions changes.
...
May 18 was almost the opposite. $137M of USDC departed Aave V3 on its own, accounting for 98 percent of the day's net outflow. On the same day, GHO supply on Aave V3 ticked up $18M. The signal reads less like depositors leaving the protocol and more like depositors rotating inside Aave's stables book, almost certainly connected to the Aave V3 USDC borrow squeeze: USDC at 97.6 percent utilization is the kind of reading that pushes the most rate-sensitive holders toward an alternative stable on the same platform.
...
Of the three largest outflow days, one fits the post-unpause LRT narrative, one was an intra-Aave stables rotation tied to the USDC squeeze, and one was dominated by structural roll-overs. The headline reading of "three large post-May-14 outflow days" obscures that they were three different phenomena. None of the three carries enough WEETH outflow to anchor the LRT-loop-finally-exited interpretation that the original framing assumed.
...
<Lead>Aave V3 sits at the center of every major thread in this issue. Five threads find their mechanism in this one protocol: the constant-price-vs-nominal accounting distinction, the USDC borrow squeeze, the LRT contraction, the sector fee compression, and the cross-protocol Chainlink dependency.</Lead>
...
The market driving Aave V3's rate signal at May close was USDC. At month-end, Aave V3 USDC carried $1.91 billion of supply against $1.87 billion of borrows, a 97.6 percent utilization that pushed supply APY to 9.62 percent and borrow APY to 10.95 percent. The market sits past its IRM kink, where the interest rate model's slope steepens sharply: at that utilization, each additional unit of borrow demand at the margin pushes supply rate up at multiple times the rate it would in a normally-utilized market. The borrow squeeze is the loudest single-market signal in DeFi lending at May close, and it is the mechanism that pulled the sector's blended stablecoin APY from 3.26 percent at April 30 to 3.60 percent at May 31, which is the mechanism that pulled the Real Yield Spread to parity, which is the macro story §03 carries.
...
What is structurally May-specific about Aave V3 is the USDC market dynamic. The LRT contraction extends an April pattern, the oracle concentration is a long-running condition, and the fee compression follows from both. The USDC squeeze formed in May, drove the rate signal that closed the Real Yield Spread at parity, and is the single most likely catalyst for whether the parity print holds in June. §08 carries that forward question.
...
What SparkLend has not developed is a rate-driven growth signal of its own. The protocol's stablecoin APYs softened in May along with the rest of the sector. The growth came from the deposit base expanding, not from rate-driven capital pull. Whether the May migration was the tail end of the April rotation or the beginning of a more durable Aave-to-SparkLend capital reallocation is the question §08 carries forward. If the Aave V3 USDC borrow squeeze persists into June and the depositors who held back from migrating in April begin to follow the WSTETH cohort to SparkLend, the protocol could find itself the natural beneficiary of two distinct rotations at once.
...
The protocol's most distinctive metric remains the liquidation engine. Across the trailing 90 days ending May 31, Fluid's effective liquidation penalty averaged 1.68 percent. SparkLend's was 4.87 percent across the same window. Morpho's was 24.10 percent. Aave V3's averaged roughly 7.0 percent, consistent with the protocol's typical bonus parameters across its collateral mix. Compound V3 and Euler V2 are not yet ingested by the liquidator-economy database, so their comparable figures are unavailable. Among the protocols measured, Fluid is structurally cheaper to be liquidated on by a wide margin. The smart-collateral architecture is the mechanism: when liquidations execute against paired-asset positions with a mathematically managed price relationship, the slippage and overshoot that drive penalty elsewhere are absent or substantially reduced.
...
The first question is the one §03 leaves directly. The Real Yield Spread closed May at parity for the first month-end since September 2025. The mechanism that closed it was stable APYs lifting to match T-bills, with the Aave V3 USDC borrow squeeze the loudest single contributor. June's spread depends on whether that mechanism persists. If USDC utilization eases from 97.6 percent and supply APYs fall back through 3.60 percent, the spread reopens negative and May reads as a transitory parity print driven by a single asset's squeeze. If the squeeze holds or extends to other markets, the spread crosses positive for the first time in eight months. Both outcomes are plausible. The data does not yet favor either.
...
The third question is the proximate mechanism behind the first two. Aave V3 USDC at 97.6 percent utilization sits past its IRM kink, where the rate model's slope steepens sharply. The market's supply APY at 9.62 percent against a 10.95 percent borrow APY is the loudest stablecoin rate signal in DeFi lending at May close. Persistence into June extends the macro story for another month. Unwind, with utilization easing as either more USDC arrives or borrows compress, would compress the rate signal and reverse the Real Yield Spread and take-rate movements that anchored §03 and §04. The market's trajectory across June's first two weeks is the cleanest forward indicator.
...
The fifth question is whether Euler V2 stabilizes or continues contracting. May's contraction was operat