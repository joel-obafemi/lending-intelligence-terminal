...
<DataTable
  ...
  rows={[
    ...
    { protocol: "Compound", flow: "−$32M", supply: "$1.40B", mom: "−6.4%" },
...
<Lead>Compound V3 runs six Comet markets on Ethereum at May 31: USDC base, USDT base, ETH (WETH) base, USDS base, wstETH base, and WBTC base. The first published cut of §06.5 covered only four Comets and mis-framed the WETH market as abandoned. The corrected reading: the WETH Comet is the third-largest by base supply and is actively used, not abandoned.</Lead>
...
The first published cut of §06.5 reported only four Comet markets and characterized the ETH (WETH) Comet at $95,000 combined as abandoned by the protocol's lending base. That reading was a snapshot artifact. Our on-chain capture script (`scripts/query-compound-comet-markets.ts`) enumerated only four of the six contracts and missed the wstETH base (`0x3D0bb1ccaB520A66e607822fC55BC921738fAFE3`) and WBTC base (`0xe85Dc543813B8c2CFEaAc371517b925a166a9293`) markets entirely. The same script's `reconciles_within_tolerance` field returned FALSE, flagging a $343M gap against the dashboard's expected reading; that flag was not actioned before publication. The corrected enumeration, drawn from Compound's deployment registry (`compound-finance/comet` deployments/mainnet folder) and the live Compound app, surfaces all six markets above.

<PullQuote>The first published cut of §06.5 was wrong: Compound's ETH Comet is the third-largest market on the protocol by base supply, not an abandoned one.</PullQuote>

The corrected reading also rotates the editorial frame. Compound V3 on Ethereum is not three big stable markets plus an empty ETH market plus a small USDS rounding error. It is six markets, three of which (USDC, USDT, ETH) carry the bulk of the activity, plus three smaller bases (wstETH, WBTC, USDS) that the protocol added more recently. The wstETH and WBTC Comets are sub-$3M combined at May 31, more launches than scaled markets. Whether they grow through June and July is the cleaner forward question than whether ETH lending got adopted, which it clearly was.
